...
def create_server(broker: Broker) -> MCPServer[Any]:
    ...
    async def pane_read(
        ...
        """Choose purpose explicitly by the immediate decision. Before typing a planned command, status checks program/prompt/pending input/running state with Luna/low and 8 lines / 1 KiB; keep its objective about input state. To answer questions or interpret output/errors, analysis uses Luna/medium and 80 lines / 64 KiB. The analysis default preserves API compatibility; it is not the recommended pre-input check. Reuse sufficient existing evidence without a redundant status call. Explicit effort overrides apply per turn. Fast is off by default; service_tier="fast" requests Fast for this call only when the user asks. service_tier_requested is not a guarantee of the served tier. requested_items specifies up to six unique answer labels for analysis only. Missing values return unknown. Use pane_excerpt for retained source evidence; partial evidence is not full proof. Every read is independent; analysis_id only groups the same task/target; expired IDs require rediscovery and a new read. Evidence belongs to the current observation, never input echo. Expand max_lines up to 1000 when needed. Raw is only for requested original text and never changes analysis context. No automatic input, retry, delta or model fallback."""

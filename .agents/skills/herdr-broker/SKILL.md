...
The user can select a Target, open details with Enter, review proposals with `a`, choose Mode with `m`, and add a same-tab Target with `n`. Existing commands, including `inspect`, `ssh-ready` and `recover`, remain available through `:`. The user can also type directly in the shared Target terminal. `quit` stops only the core; users close individual panes when finished. Preserve running cores when updating the project; the dashboard applies on their next start.

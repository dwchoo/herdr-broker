...
export async function consoleProcess(t, h, { tty = true, clockPath } = {}) {
  await h.core.close();
  const args = ['test/process-fixture.mjs', 'serve', h.endpoint, join(h.root, 'state'), ...(clockPath ? [clockPath] : [])];
  const child = tty ? spawn('python3', ['test/pty-fixture.py', process.execPath, ...args], { detached: true, stdio: 'pipe' }) : spawn(process.execPath, args, { detached: true, stdio: 'pipe' });

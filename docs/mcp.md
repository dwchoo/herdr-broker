...
| `pane_read` | `pane_id`, `terminal_id`, optional `objective`, `raw=false`, `offset=0`, `effort="low"`, `max_lines` | Default Worker report or user-requested bounded raw text |
...
`pane_read` captures a bounded recent screen and invokes `gpt-5.6-luna` (default effort `low`) through the official Codex Python SDK even for short output. Analysis defaults to the latest 80 lines; set `max_lines` (1–1,000) and `effort` (`low`, `medium`, `high`) explicitly when needed. Raw reads retain their 1,000-line default. All reads retain the 64 KiB capture limit. Pass the user's actual question in `objective` so the first observation checks current program/pending input and existing relevant results together. Reuse sufficient existing output when freshness permits; otherwise execute and inspect recent output and prompt return. Expand the range or effort when evidence is insufficient; no automatic escalation or replay occurs. Recent reads are fresh tail windows, not append-only deltas; no screen cache is kept.

Reads disclose the selected range and truncation. Analysis responses also provide `timings_ms` for capture, SDK startup, turn submission, first event, remaining stream, validation, cleanup, final identity check and total service time. Total time covers the Broker read method, excluding initial MCP context verification, transport and Parent processing. Worker admission has no queue. Provider queue, input processing and reasoning times cannot be separated by this SDK; do not infer them from first-event time. Worker failure timing diagnostics on stderr contain no screen/objective/report. Reports contain a concise Korean summary, findings with evidence from supplied lines, suggested checks and uncertainties. Worker tools, filesystem access and network tools are disabled. Worker failures/cancellation are explicit; never silently return raw text or change models. Raw reads are for explicit user requests for original text, with limits and truncation disclosed. Offsets are within the current capture, not durable scrollback cursors.

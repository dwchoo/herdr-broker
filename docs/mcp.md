...
| `analysis_release` | `analysis_id` | Release an idle analysis context; unknown IDs are a harmless no-op |
| `pane_read` | `pane_id`, `terminal_id`, optional `objective`, `raw=false`, `offset=0`, `purpose="analysis"`, optional `effort`, `max_lines`, `analysis_id` | Default Worker report or user-requested bounded raw text |
...
`pane_read` analyzes even short screens with `gpt-5.6-luna`. `purpose="analysis"` (default) selects high effort; `purpose="status"` selects low for prompt return, pending input, running state and the presence of relevant existing output. Explicit `effort` (`low`, `medium`, `high`) overrides that default per turn. Status uses the same report shape, capped at 1 KiB and two findings; analysis retains 4 KiB. Analysis defaults to 80 recent lines, expandable with `max_lines` (1–1,000), with a 64 KiB capture cap. Raw retains its 1,000-line default and never updates analysis context. Pass the actual user objective, reuse sufficient existing results when freshness permits, and verify output/errors/prompt rather than input echo. Each capture is a fresh tail window, not a complete delta; no screen cache is kept.

Omit `analysis_id` for a new task. Continue the same task on the exact workspace/pane/terminal with the returned ID, and call `analysis_release` when done. Another target is rejected (`analysis_target_mismatch`); expired, evicted or recycled IDs return `analysis_session_expired`. Rediscover and start without that ID after expiration. The response adds `analysis_id`, `observation_id`, `sdk_reused`, `context_reused`, `context_reset` and `context_reset_reason`. Evidence keys include the observation ID and must refer to this capture. A prior thread answer is not current screen evidence.

Reads disclose the selected range and truncation. Analysis responses also provide `timings_ms` for capture, SDK readiness wait (`sdk_start`), thread creation (`thread_start`), turn submission, first event, remaining stream, validation, cleanup, final identity check and total service time. Total time covers the Broker read method, excluding initial MCP context verification, transport and Parent processing. Worker admission has no queue. Provider queue, input processing and reasoning times cannot be separated by this SDK; do not infer them from first-event time. Worker failure timing diagnostics on stderr contain no screen/objective/report. Reports contain a concise Korean summary, findings with evidence from supplied lines, suggested checks and uncertainties. Worker tools, filesystem access and network tools are disabled. Worker failures/cancellation are explicit; never silently return raw text or change models. Raw reads are for explicit user requests for original text, with limits and truncation disclosed. Offsets are within the current capture, not durable scrollback cursors.
...
Known Herdr enqueue rejections (`invalid_key`, `pane_not_found`, `pane_send_failed`) return `rejected`; a failure before submission returns `not_sent`. SDK startup and cleanup failures are explicit Worker errors too. Screen responses must match the requested source and format, with a nonnegative integer revision and boolean truncation flag. The output schema restricts evidence IDs to the current observation and its actual line IDs; the Worker checks each referenced line again before returning a report.

## Labels, context and lifecycle

A verified MCP starts one SDK in the background without a model turn. Metadata/input tools do not wait for it; early analysis waits for the shared initialization. The healthy process stays alive until MCP shutdown: no idle shutdown or lifetime thread-count recycling. The SDK holds bounded ephemeral task context; the Broker does not duplicate screen/report histories or store completed Jobs. Global notifications and turn streams are drained.

Admission precedes screen capture: at most two analyses and two retained contexts, with no queue. A simultaneous continuation fails with `analysis_session_busy`. A new task evicts the oldest idle context; two busy contexts return `worker_busy`. Release contexts after five minutes idle. Rotate their thread before a ninth turn or when adding the next input would exceed 128 KiB cumulative screen/question/report payload; report the reset without summarization. SDK unsubscribe does not prove immediate unload.

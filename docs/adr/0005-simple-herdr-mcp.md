...
`pane_read`는 출력 길이에 관계없이 Codex Python SDK의 `gpt-5.6-luna` Worker로 분석한다. 후속 지연 개선 요청에 따라 기본은 최근 80줄·low이며 Parent가 목적에 맞게 읽기 범위와 effort를 높일 수 있다. 기존 high 고정 결정을 변경하며 Worker 기본 사용은 유지한다. 사용자가 원문을 요청한 때만 direct 읽기를 사용한다. Worker는 전달된 화면만 읽고 파일·shell·MCP 도구를 사용할 수 없다. SDK의 runtime·취소·도구 제한을 실제 검증한다. 실패 시 raw나 다른 모델로 자동 대체하지 않는다.

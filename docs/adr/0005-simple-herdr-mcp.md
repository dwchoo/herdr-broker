...
`pane_read`는 출력 길이에 관계없이 Codex Python SDK의 `gpt-5.6-luna` Worker로 분석한다. 후속 지연 개선은 analysis 최근 80줄, status 최근 8줄·1 KiB와 목적별 effort를 사용한다. 부족한 상태 문맥은 Parent가 명시적으로 확장하며 [입력 축소](../implementation/status-input.md)의 짧은 Worker 형식과 계측을 따른다. analysis 기본은 Luna/high, status는 Luna/low이며 명시적 effort를 유지한다. MCP 시작 시 하나의 SDK를 준비·유지하고 같은 작업의 ephemeral thread를 제한적으로 재사용한다. 수명·자원·검증 기준은 [SDK 재사용](../implementation/sdk-reuse.md)에 따른다. 사용자가 원문을 요청한 때만 direct 읽기를 사용한다. Worker는 전달된 화면만 읽고 파일·shell·MCP 도구를 사용할 수 없다. SDK의 runtime·취소·도구 제한을 실제 검증한다. 실패 시 raw나 다른 모델로 자동 대체하지 않는다.

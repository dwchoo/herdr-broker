...
> 후속 결정: 단순 상태 확인은 **Luna/low**, 로그 해석·진단·종합 분석은 **Luna/high**로 구분한다. 아래 내용은 이미 구현한 일괄 low 변경의 기록이며, SDK/thread 재사용과 후속 기본값 변경은 [수명 관리 검토](sdk-reuse-lifecycle-analysis.md)를 따른다.

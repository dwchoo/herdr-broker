...
This document describes the preserved TypeScript Console entrypoint only. The default Python server follows the [current MCP contract](mcp.md).

The project skill opens or attaches a persistent background core inside Herdr. Shared Targets remain ordinary shells; an on-demand management pane connects to that core without owning its lifetime. One Parent attaches at a time. Existing UUID and pane-ID calls remain valid, with four-digit console_code and pane_code addresses added for humans.

`pane_list` defaults to the verified caller’s entire Herdr workspace, across tabs, even before attachment. `scope: "broker"` lists owned identities including missing/moved/replaced Targets. Pages include names, codes, process/cwd metadata, ownership, roles, availability and last successful check. Listing never captures output, starts Jobs/Workers, sends input, renames or registers terminals. Lookup failure is explicit, not an empty success.

Interpret incomplete numbers and contextual references in the Parent using current inventory and conversation. No prefix, edit-distance or confidence-threshold resolver chooses a target. After choosing, use the exact returned identity; execution still validates ownership, current tab/terminal and Action policy.

Console membership binds exact terminal identities within one tab. Explicit registration may adopt an existing same-tab terminal without restarting it; another Broker’s Target, a Codex pane or a management pane cannot be adopted. Metadata visibility does not grant input scope. Parent connections can explicitly detach/switch within the tab, but active Action submission/observation blocks switching. Old commands are never replayed.

Production entrypoints verify the project, injected Herdr environment, canonical socket, live pane identity and OS ancestry. Background launch completes verification while its launcher is still alive. The private management channel requires the registered management pane and a capability never returned by Parent MCP. Approval and Mode increases stay in the actual interactive management client. Same-user direct socket/filesystem isolation remains out of scope.

## Discovery and lifecycle tools

- `pane_list({scope?: "workspace" | "broker", cursor?})`: current metadata pages; `error` distinguishes failed lookup from an empty result. `checked_at` is the last successful verification. Follow `next` when `truncated` is true. Metadata visibility never grants execution authority.
- `console_list({cursor?})`: codes, ownership and connection summaries for this project; runtime presence includes its check time and becomes unconfirmed after missed heartbeats.
- `console_attach({console_id})`, `console_detach({})`: exact UUID/four-digit connection and explicit release. In-progress Action wire submission or observation blocks release/switch.
- `pane_register({pane_id, name?})`, `pane_rename({pane_id, name})`: exact native ID or four-digit address. No approximate resolution.
- `pane_create({})`: add a shared shell in the same tab.
- `console_manage({})`: open/reuse the temporary management pane. `quit` closes that view; interactive `stop` stops only the Broker. Shells survive both.

Use the selected row's exact native `pane_id` and `terminal_id` in existing diagnosis/Action calls. Four-digit codes are addresses, not authorization. The OS-user registry never recycles an address; exhaustion fails explicitly.
...
Setup writes only this checkout's MCP configuration; a new Codex launched in this project inherits it. The Parent helper provides the same settings for one invocation and uses `on-request` approval policy with the existing reviewer. Codex tool approval and Broker Action Mode both apply. Both paths forward Herdr-injected context for process ancestry validation. See [legacy operations](operations-legacy.md) for persistence and restart behavior.
...
| `console_list` | optional returned `cursor`; list this project’s Console IDs and workspace/tab IDs in pages of at most 8 records / 8 KiB; each row previews at most 8 Targets with pane_count/panes_truncated |

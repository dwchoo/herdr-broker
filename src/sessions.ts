...
export class Sessions {
  ...
  async observe(pane: Pane, signal?: AbortSignal, actionTarget = false) {
    ...
    const fingerprint = JSON.stringify([target, info.shell_pid]);
    ...
    const session: Session = { id: randomUUID(), target, process: info, fingerprint, mode: 2, revision: 1, active: true, bytes };
    this.current.set(pane.pane_id, session);
    return session;
  }
  get(id: string) {
    const session = [...this.current.values()].find(value => value.id === id && value.active);

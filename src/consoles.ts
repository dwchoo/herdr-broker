...
const recordSchema = z.strictObject({
  ...
  workspace_id: z.string().min(1).max(256), controller: target, panes: z.array(target).min(1).max(8), created_at: z.string().datetime(),
});
export type ConsoleRecord = z.infer<typeof recordSchema>;
export interface ConsoleConfiguration { endpoint: string; stateRoot: string; project: string }
...
export class Consoles {
  ...
  async get(id: string): Promise<ConsoleRecord> {
    try {
      ...
      return record;
    } catch { throw new BrokerError('console_unavailable'); }
  }
  async list(cursor?: string) {
    ...
    for (const file of files.filter(file => /^[0-9a-f-]{36}\.json$/.test(file))) {
      try {
        ...
        records.push({ console_id: record.console_id, label: record.label, created_at: record.created_at });
  ...
  async verify(record: ConsoleRecord) {
    ...
    if (controller.workspace_id !== record.workspace_id || controller.terminal_id !== record.controller.terminal_id) throw new BrokerError('console_workspace_changed');
  }
  ...
  async create(label: string) {
    ...
    const controller = await this.herdr.createWorkspace(`Broker · ${label}`, this.config.project);
    const pane = await this.herdr.createTerminal(controller.workspace_id, this.config.project, controller.pane_id);
    const identity = ({ pane_id, terminal_id }: Pane) => ({ pane_id, terminal_id });
    const record = recordSchema.parse({ console_id: consoleId, label, project: this.config.project, endpoint: this.config.endpoint, workspace_id: controller.workspace_id, controller: identity(controller), panes: [identity(pane)], created_at: new Date().toISOString() });
  ...
  async addTerminal(record: ConsoleRecord) {
    ...
    const pane = await this.herdr.createTerminal(record.workspace_id, record.project);

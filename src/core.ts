...
import { z } from 'zod';
import { BrokerError, Herdr, type ConsoleScope } from './herdr.js';
...
export async function startCore(options: CoreOptions) {
  ...
  const server = createServer(socket => {
    ...
    const handle = serveStdio(() => {
      ...
      if (options.consoleId) mcp.registerTool('console_status', { annotations: { readOnlyHint: true, destructiveHint: false }, description: 'Read owned terminals and bounded durable receipts before continuing in this Console.', inputSchema: z.strictObject({}) }, () => safe(consoleStatus));
  ...
  return { socketPath, close, ...(options.consoleId && { consoleStatus }), summary: () => { const status = jobs.summary(); return { ...(options.consoleId && { console: consoleStatus() }), ...status, jobs: status.jobs.map(job => ({ ...job, ...actions.budget(job.job_id) })), ...sessions.summary(), ...actions.summary() }; }, actions, purge: (id: string) => { authority.verify(); return jobs.purge(id); } };
}

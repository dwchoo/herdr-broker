  ...
  const server = createServer(socket => {
    ...
    const handle = serveStdio(() => {
      ...
      mcp.registerTool('evidence_get', {
        ...
        description: 'Read bounded redacted Evidence beginning at one immutable row ID, including up to 16 following rows. Follow next only when truncated; this is excerpt pagination, not Snapshot history completeness.',

...
export async function doctor(options: CoreOptions) {
  ...
  return { ok: runtime.supported && herdr.ok && worker.ok && storage.ok, runtime, herdr, worker, state: storage, profiles: { passive: herdr.ok, local_posix: herdr.ok, ssh_posix: false }, limits: { parent_payload_bytes: 16384, worker_calls: 4, ordinary_actions: 3, interrupts: 1 }, privacy: { mode: 'broker_memory', complete_no_store: false }, pane_input_attempts: 0 };
}
